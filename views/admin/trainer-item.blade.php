    ...
    <table class="table table-hover">
        ...
        <thead>
            <tr style="height: 50px;">
                ...
                <!--STATUS-->
                <?php $name = 'trainer_status'; ?>

                <th class="hidden-xs" style='width:{{ $withs['status'] }}'>{!! trans($plang_admin . '.columns.trainer-status') !!}
                    <a href='{!! $sorting['url'][$name] !!}' class='tb-id' data-order='asc'>
                        @if ($sorting['items'][$name] == 'asc')
                            <i class="fa fa-sort-alpha-asc" aria-hidden="true"></i>
                        @elseif($sorting['items'][$name] == 'desc')
            ...
            @foreach ($items as $item)
                <tr>
                    ...
                    <td style="text-align: center;">
                        ...
                        @if ($item->trainer_status && isset($status['list'][$item->trainer_status]))
                            <i class="fa fa-circle" style="color:{!! $status['color'][$item->trainer_status] !!}"
                                title='{!! $status['list'][$item->trainer_status] !!}'></i>
                        @else
                    ...
                    <td>
                        ...
                        @if (Route::has('comments.by_context'))
                            <a href="{!! URL::route('comments.by_context', ['id' => $item->id, 'context' => 'trainer']) !!}">
                                <i class="fa fa-commenting" aria-hidden="true"></i>
                            </a>&nbsp;
                        @endif

                        <!--edit-->
                        <a href="{!! URL::route('trainer.edit', ['id' => $item->id]) !!}">
                            <i class="fa fa-edit f-tb-icon"></i>
                        </a>&nbsp;


                        <!--copy-->
                        <a href="{!! URL::route('trainer.copy', ['cid' => $item->id]) !!}" class="margin-left-5">
